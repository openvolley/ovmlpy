...
The first time you use `ovmlpy`, it will download and set up Python in a virtual environment for use by this package. The first time use a new type of network, it will also download the associated network weights file.

## Example

Construct the network. The first time this function is run for each type of network, it will download and cache the network weights file (~70MB, depending on network version).
